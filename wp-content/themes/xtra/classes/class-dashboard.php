...
class Codevz_Core_Dashboard extends Codevz_Core_Theme {
	...
	public function init() {
		...
		if ( self::option( 'white_label_exclude_admin' ) && function_exists( 'current_user_can' ) && current_user_can( 'administrator' ) ) {
			$this->disable = [];
		}
	...
	public function admin_head() {
		...
		echo '<style>';
		echo 'a.wp-first-item[href*="theme-activation"] {
	display: none !important
}

#adminmenu li li a[href*="theme-activation"],
.xtra-dashboard-menus a[href*="theme-activation"]:not(:first-child) {
	...
	private function render_before( $active = null ) {
		...
		echo '<div class="xtra-dashboard-header">';
			...
			if ( ! isset( $this->disable[ 'envato' ] ) ) {
				...
				echo wp_kses_post( apply_filters( 'codevz_buy_market', '<a href="' . esc_url( apply_filters( 'codevz_config_buy_link', '#' ) ) . '" class="xtra-market" target="_blank"><img src="' . esc_url( self::$url ) . 'assets/img/envato.png" /></a>' ) );
		...
		foreach( $this->menus as $slug => $title ) {
			...
			echo '<a href="' . esc_url( admin_url( $link ) ) . '" class="' . esc_attr( $active === $slug ? 'xtra-current' : '' ) . '"><img src="' . esc_url( self::$url ) . 'assets/img/' . esc_attr( $img ) . '.png" /><span>' . strip_tags( $title ) . '</span>' . wp_kses_post( $additional ) . '</a>';
		...
		if ( ! isset( $this->disable[ 'docs' ] ) ) {
			...
			echo '<a href="' . esc_url( apply_filters( 'codevz_config_docs', '#' ) ) . '" target="_blank"><img src="' . esc_url( self::$url ) . 'assets/img/docs.png" /><span>' . esc_html( Codevz_Core_Strings::get( 'documentation' ) ) . '</span></a>';

		}

		if ( ! isset( $this->disable[ 'youtube' ] ) ) {

			echo '<a href="' . esc_url( apply_filters( 'codevz_config_youtube', '#' ) ) . '" target="_blank"><img src="' . esc_url( self::$url ) . 'assets/img/videos.png" /><span>' . esc_html( Codevz_Core_Strings::get( 'video_tutorials' ) ) . '</span></a>';

		}

		if ( ! isset( $this->disable[ 'changelog' ] ) ) {

			echo '<a href="' . esc_url( apply_filters( 'codevz_config_changelog_link', '#' ) ) . '" target="_blank"><img src="' . esc_url( self::$url ) . 'assets/img/changelog.png" /><span>' . esc_html( Codevz_Core_Strings::get( 'change_log' ) ) . '</span></a>';

		}

		if ( ! isset( $this->disable[ 'ticksy' ] ) ) {

			echo '<a href="' . esc_url( apply_filters( 'codevz_config_support_link', '#' ) ) . '" target="_blank"><img src="' . esc_url( self::$url ) . 'assets/img/support.png" /><span>' . esc_html( Codevz_Core_Strings::get( 'support' ) ) . '</span></a>';

		}

		if ( ! isset( $this->disable[ 'faq' ] ) ) {

			echo '<a href="' . esc_url( apply_filters( 'codevz_config_faq_link', '#' ) ) . '" target="_blank"><img src="' . esc_url( self::$url ) . 'assets/img/faq.png" /><span>' . esc_html( Codevz_Core_Strings::get( 'faq' ) ) . '</span></a>';
	...
	public function plugins() {
		...
		foreach( $this->plugins as $slug => $plugin ) {
			...
			echo '<div class="xtra-plugin">';
				...
				echo '<img src="' . esc_url( self::$url ) . 'assets/img/' . esc_attr( self::contains( $slug, 'codevz-plus' ) ? 'codevz-plus' : $slug ) . '.jpg" alt="' . esc_attr( $plugin[ 'name' ] ) . '" />';
	...
	public function importer() {
		...
		if ( self::$premium && $demos_count > 21 ) {
