...
if ( ! class_exists( 'Codevz_Core_Theme' ) ) {
	...
	class Codevz_Core_Theme {
		...
		public static function theme_setup() {
			...
			// Temp fix for elementor SVG icons.
				...
				if ( function_exists( 'is_woocommerce' ) ) {
					...
					if ( $button ) {
						...
						if ( isset( $matches[1] ) ) {
							...
							$css .= '.woocommerce div.product form.cart .variations .codevz-variations-button label{border-radius:' . esc_html( $matches[1] ) . '}';
		...
		public static function get_row_element( $i, $m = [] ) {
			...
			if ( $elm === 'logo' || $elm === 'logo_2' ) {
				...
				echo '<a class="' . $elm . '_icon" href="' . esc_url( self::$home_url . ( $elm === 'wishlist' ? 'wishlist' : 'products-compare' ) ) . '" data-title="' . esc_attr( $shopcart_tooltip ) . '" style="' . esc_attr( $container ) . '"><i class="' . esc_attr( $icon ) . '" style="' . esc_attr( $icon_style ) . '" aria-hidden="true"></i><span>' . do_shortcode( esc_html( $shopcart_title ) ) . '</span></a>';
		...
		public static function generate_header() {
			...
			if ( ! file_exists( str_replace( '-child', '', self::$dir ) . '-child/header.php' ) ) {
				...
				if ( $hidden_top_bar && $hidden_top_bar !== 'none' ) {
					...
					echo '</div><i class="' . esc_attr( self::option( 'hidden_top_bar_icon', 'fa fa-angle-down' ) ) . '" aria-hidden="true"></i></div>';
				...
				if ( $cover && $cover !== 'none' ) {
					...
					if ( $cover === 'rev' && $cover_rev ) {
						...
						if ( $slider && ! self::contains( $slider, '[rev_slider alias=' ) ) {
		...
		public static function generate_page( $page = '' ) {
			...
			if ( $is_404 ) {
				...
				if ( $page_404 ) {
					...
					echo '<a class="button" href="' . esc_url( $url ) . '" style="margin: 80px auto 0;display:table">' . do_shortcode( esc_html( self::option( '404_btn', 'Back to Homepage' ) ) ) . '</a>';
		...
		public static function breadcrumbs( $is_right = '', $hide_current = false ) {
			...
			foreach( $bc as $ancestor ) {
				...
				if ( $i === $count ) {
					global $wp;
					$out[] = '<b itemscope itemtype="http://data-vocabulary.org/Breadcrumb" class="inactive_l"><a class="cz_br_current" href="' . trailingslashit( trailingslashit( get_home_url() ) . $wp->request ) . '" onclick="return false;" itemprop="url"><span itemprop="title">' . wp_kses_post( $ancestor['title'] ) . '</span></a></b>';
				} else {
					$out[] = '<b itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . esc_url( $ancestor['link'] ) . '" itemprop="url"' . ( isset( $ancestor[ 'home' ] ) ? ' title="' . esc_attr( get_bloginfo( 'name' ) ) . '"' : '' ) . '><span itemprop="title">' . wp_kses_post( $ancestor['title'] ) . '</span></a></b>';
				}
		...
		public static function breadcrumbs_array() {
			...
			$bc = [];
			$bc[] = [ 'home' => true, 'title' => '<i class="' . esc_attr( self::option( 'breadcrumbs_home_icon', 'fa fa-home' ) ) . ' cz_breadcrumbs_home" aria-hidden="true"></i>', 'link' => trailingslashit( get_home_url() ) ];
