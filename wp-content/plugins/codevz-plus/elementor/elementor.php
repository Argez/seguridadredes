...
class Xtra_Elementor {
	...
	public function categories( $elements_manager ) {
		...
		$elements_manager->add_category(
			'xtra',
			[
				'title' => esc_html__( 'Codevz Plus', 'plugin-name' ),
				'icon' => 'fa fa-plug',
			]
		);
	...
	public function icons_manager( $tabs = [] ) {
		...
		$tabs[ 'xtra-custom-icons' ] = array(
			...
			'label'         => esc_html__( 'Custom theme icons', 'codevz' ) . ( $free ? ' [' . esc_html__( 'PRO', 'codevz' ) . ']' : '' ),
	...
	public function after_section_end( $section, $section_id, $args ) {
		...
		if ( $section->get_name() && $section_id === 'section_effects' ) {
			...
			$section->start_controls_section(
				...
				[
					...
					'label' => esc_html__( 'Block Reveal Effect', 'codevz' ),
				]
			);
			...
			$section->add_control(
				...
				[
					...
					'options' 	=> [
						...
						'down' 		=> esc_html__( 'Down', 'codevz' ),
						'up' 		=> esc_html__( 'Up', 'codevz' ),
					],
		...
		if ( ( $section->get_name() === 'section' && $section_id === 'section_advanced' ) || ( $section->get_name() === 'container' && $section_id === 'section_effects' ) ) {
			...
			$section->start_controls_section(
				...
				[
					'label' 	=> esc_html__( 'Background particles', 'codevz' ),
					'tab' 		=> Controls_Manager::TAB_ADVANCED
				]
			);
		...
		if ( $column_sk || $container_sk ) {
			...
			$section->start_controls_section(
				'xtra_column_sks',
				[
					'label' 	=> esc_html__( 'StyleKits', 'codevz' ),
			...
			$section->start_controls_section(
				...
				[
					'label' 	=> esc_html__( 'More advanced', 'codevz' ),
					'tab' 		=> Controls_Manager::TAB_ADVANCED
				]
			);
		...
		if ( Codevz_Plus::is_free() ) {
			...
			$widget->add_control(
				...
				[
					...
					'raw' 		=> self::pro_section( esc_html__( 'Tilt effect', 'codevz' ) )
				]
			);
	...
	public static function parallax_settings( $widget, $repeater = false ) {
		...
		if ( ! $repeater ) {
			...
			$widget->start_controls_section(
				'section_parallax',
				[
					'label' => esc_html__( 'Parallax effect', 'codevz' )
				]
			);
			...
			if ( ! $repeater ) {
				...
				$widget->add_control(
					...
					[
						'label' 	=> '',
						'type' 		=> Controls_Manager::RAW_HTML,
						'raw' 		=> self::pro_section( esc_html__( 'Parallax effect', 'codevz' ) )
					]
				);
	...
	public static function posts_grid_items( $settings = '', $out = '' ) {
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$out .= '<a class="cz_grid_link' . $img_fx . $issvg . '" href="' . get_the_permalink() . '" title="' . get_the_title() . '"' . $settings['tilt_data'] . '>';
	...
	public function login_register() {
		...
		if ( $pass_r ) {
			...
			if( $update_user ) {
				...
				$to = $user->user_email;
				$subject = esc_html__( 'Your new password', 'codevz' );
				$sender = 'From: '.get_bloginfo('name').' <'.$from.'>' . "\r\n";

				$message = esc_html__( 'Your new password', 'codevz' ) . ' <strong>' . $pass . '</strong><br /><br /><a href="' . get_home_url() . '/' . '">' . get_home_url() . '</a>';
		...
		// Registration
			...
			if ( is_wp_error( $user ) ){	
				...
				$from = 'do-not-reply@'.preg_replace( '/^www\./', '', $_SERVER['SERVER_NAME'] ); 
				$subject = esc_html__( 'Thank you for resigtration', 'codevz' );
				$sender = 'From: '.get_bloginfo('name').' <'.$from.'>' . "\r\n";
				
				$message = '<h4>' . esc_html__( 'Thank you for resigtration', 'codevz' ) . '</h4><br /><ul>
					<li>' . esc_html__( 'Username', 'codevz' ) . ' <strong>' . $username . '</strong></li>
					<li>' . esc_html__( 'Password', 'codevz' ) . ' <strong>' . $password . '</strong></li>
