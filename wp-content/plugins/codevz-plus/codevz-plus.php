 ...
 * Description: Elements, Widgets, StyleKit, custom post types, options and page builder features.
 * Version: 4.8.5
 * Author: Codevz
...
class Codevz_Plus {
	...
	protected function __construct() {
		...
		add_filter( 'template_redirect', [ $this, 'maintenance_mode' ], 99 );
		...
		if ( ! self::option( 'search_cpt' ) ) {
			add_filter( 'posts_where', [ $this, 'search_where' ], 11 );
			add_filter( 'posts_join', [ $this, 'search_join' ], 11 );
			add_filter( 'posts_groupby', [ $this, 'search_groupby' ], 11 );
		}
	...
	public static function get_page_by_title( $page_title, $post_types = [ 'page', 'elementor_library' ] ) {
	...
	public static function is_free( $is_free = false ) {
		...
		if ( ! isset( $code[ 'purchase_code' ] ) || ( isset( $code[ 'purchase_code' ] ) && self::contains( $code[ 'purchase_code' ], [ '*', 'xxx' ] ) ) ) {
	...
	public static function pro_badge( $link = true ) {
		...
		if ( $link ) {
			...
			return '<a href="' . esc_url( get_admin_url() ) . 'admin.php?page=theme-activation" data-tooltip="' . esc_html__( 'Activate your theme with purchase code to access this feature.', 'codevz' ) . '" target="_blank" class="xtra-pro"><span">' . esc_html__( 'PRO', 'xtra' ) . '</span></a>';

		} else {

			return '<span class="xtra-pro"><span">' . esc_html__( 'PRO', 'xtra' ) . '</span></span>';
	...
	public static function pro_message( $prefix = '' ) {
		...
		return is_user_logged_in() ? '<div class="xtra-inactive-notice"><span>' . ( $prefix ? $prefix . ', ' : '' ) . esc_html__( 'Activate your theme to access all PRO features.', 'codevz' ) . '</span></div>' : '';
	...
	public static function share() {
		...
		foreach( $share as $name ) {
			...
			if ( isset( $url[ $name ] ) ) {
				...
				echo '<a href="' . $url[ $name ]['url'] . '" rel="noopener noreferrer nofollow" class="cz-' . ( ( $name === 'twitter' ) ? 'x-' : '' ) . $name . '" ' . ( $tooltip ? 'data-' : '' ) . 'title="' . esc_attr( $title_prefix . ' ' . $url[ $name ]['title'] ) . '" aria-label="' . esc_attr( $title_prefix . ' ' . $url[ $name ]['title'] ) . '"' . $custom_data . '><i class="' . $ico . '"></i><span>' . esc_html( $url[ $name ]['title'] ) . '</span></a>';
	...
	public static function body_class( $c = [] ) {
		...
		// RTL
		$c[] = self::$is_rtl ? 'rtl' : '';
		$c[] = self::option( 'disable_rtl_numbers' ) ? 'xtra-disable-rtl-numbers' : '';
		...
		// No fade on load.
		$c[] = self::option( 'first_load_fade' ) ? '' : 'xtra-no-fade';
	...
	public function wp_footer() {
		...
		if ( $cf7 ) {
			...
			if ( $cf7_link && ! $cf7 ) {
				...
				echo '<a href="' . $cf7_link . '" target="_blank" aria-label="Contact">' . wp_kses_post( $icon ) . '</a>';
		...
		if ( empty( $_COOKIE[ 'xtra_cookie' ] ) ) {
			...
			if ( $cookie ) {
				...
				echo '<a href="#" class="cz_btn xtra-cookie-button" aria-label="Close">' . do_shortcode( wp_kses_post( $button ) ) . '</a>';
				echo '</div>';
		...
		if ( self::option( 'mobile_fixed_navigation' ) ) {
			...
			foreach ( $items as $item ) {
				...
				if ( isset( $item['icon'] ) ) {
					...
					echo '<a href="' . esc_url( do_shortcode( $item['link'] ) ) . '" title="' . esc_attr( do_shortcode( $item['title'] ) ) . '">' . wp_kses_post( $icon ) . '<span>' . wp_kses_post( do_shortcode( $item['title'] ) ) . '</span></a>';
	...
	public function admin_init() {
		...
		if ( ! empty( $_GET[ 'codevz_edit_content' ] ) ) {

			$page = esc_attr( $_GET[ 'codevz_edit_content' ] );

			if ( is_numeric( $page ) ) {

				$page_id = esc_attr( $_GET[ 'codevz_edit_content' ] );

			} else {

				$page_id = self::get_page_by_title( esc_attr( $_GET[ 'codevz_edit_content' ] ) );
	...
	public function init() {
		...
		// Plugin title.
		self::$title = esc_html__( 'Codevz plus elements', 'codevz' );

		// Strings.
		$plugin_name = esc_html__( 'Codevz Plus', 'codevz' );
		$description = esc_html__( 'StyleKit, custom post types, options and page builder elements.', 'codevz' );
	...
	public static function vc_param_cz_presets( $s, $v ) {
		...
		if ( self::is_free() ) {
			...
			return self::pro_message( esc_html__( 'To access all the premium presets of this element', 'codevz' ) );
	...
	public static function wp_enqueue_scripts() {
		...
		if ( self::$is_rtl && ! self::option( 'disable_rtl_numbers' ) ) {
		...
		<script>
			...
			jQuery( function( $ ) {
				...
				setTimeout(function() {
					...
					$( '#wpb_tinymce_content_ifr' ).contents().find( 'head' ).append( '<style>.cz_highlight_1 {position: relative}.cz_highlight_1:after {width: calc(100% + 6px);position: absolute;content: "";height: 10px;bottom: 0;left: -3px;background: "<?php esc_html_e( self::option( 'site_color' ) ); ?>";z-index: -1;opacity: .2;}</style>' );
	...
	public static function tiny_mce_before_init( $i ) {
		...
		$websafe_fonts = self::web_safe_fonts();
		...
		$style_formats = array(
			...
			array(
				'title'  => esc_html__( 'Half Highlight', 'codevz' ),
				'inline' => 'span',
	...
	public static function _out( $a, $c = '', $s = '', $enqueue = '', $enqueue_extra = '' ) {
		...
		// Check free version.
		if ( self::is_free() && self::$vc_editable && ! isset( $_GET['preview_id'] ) ) {

			return self::pro_message( '<b>' . ucwords( str_replace( [ 'cz_', '_' ], [ '', ' ' ], $enqueue ) ) . '</b> ' . esc_html__( 'is a PRO element', 'codevz' ) );
	...
	public static function lazyload( $c ) {
		...
		if ( self::option( 'lazyload' ) == 'true' ) {
			...
			foreach ( $matches as $key ) {
				if ( isset( $key[0] ) && ! self::contains( $key[0], [ 'codevz-ignore-lazyload', 'data:image', 'data-thumb', 'data-bg', 'data-ww=', 'rev-slide', 'xmlns' ] ) ) {
	...
	/**
	 * file_get_contents for theme usage.
	 * 
	 * @return object
	 */
	...
	public function search_where( $where ) { 
		...
		if ( is_search() && ! is_admin() && empty( $_GET[ 'prcat' ] ) ) {
	...
	public static function ajax_search() {
		...
		// Search stats for plugin compatibility. 
		$visit = wp_remote_get( get_search_link( $s ) );
		...
		if ( ! empty( $posts ) ) {
			...
			foreach( $posts as $post_id ) {
				...
				echo '<span class="cz_search_item_cpt mr10"><i class="fa fa-folder-o mr4"></i>' . ucwords( ( $cpt === 'dwqa-question' ) ? 'Questions' : $cpt ) . '</span>';
			...
			if ( $i === 0 ) {
				...
				unset( $_GET['action'] );
				unset( $_GET['nonce'] );
				echo '<a class="va_results" href="' . trailingslashit( esc_url( get_home_url() ) ) . '?' . http_build_query( $_GET ) . '"> ... </a>';
	...
	public static function post_types() {
		...
		foreach ( $post_types as $cpt ) {
			...
			$opt = array(
				...
				'title' 		=> empty( $options[ 'title_' . $cpt ] ) ? ( $cpt === 'portfolio' ? esc_html__( 'Portfolio', 'codevz' ) : ucwords( $cpt ) ) : $options[ 'title_' . $cpt ], 
				'cat_slug' 		=> empty( $options[ 'cat_' . $cpt ] ) ? $cpt . '/cat' : $options[ 'cat_' . $cpt ], 
				'cat_title' 	=> empty( $options[ 'cat_title_' . $cpt ] ) ? esc_html__( 'Categories', 'codevz' ) : $options[ 'cat_title_' . $cpt ], 
				'tags_slug' 	=> empty( $options[ 'tags_' . $cpt ] ) ? $cpt . '/tags' : $options[ 'tags_' . $cpt ], 
				'tags_title' 	=> empty( $options[ 'tags_title_' . $cpt ] ) ? esc_html__( 'Tags', 'codevz' ) : $options[ 'tags_title_' . $cpt ]
			);
